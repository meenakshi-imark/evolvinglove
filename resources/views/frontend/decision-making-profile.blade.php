            ...
            <div class="page-content">
                <div class="sitemax-width">
                    <div class="page-breadcrum mb-6">
                        <a href="/communication-profile" class="btn btn-secondary prev">
                        ...
                        <a href="/parenting-profile" class="btn btn-secondary next">
                            Next <i class="fas fa-caret-right"></i>
                        </a>
                    </div>

                    <div class="text-center mb-5">
                        <h2 class="color_black mb-4">DECISION MAKING PROFILE</h2>
                    ...
                    @if($emp2->type=="directive")
                    @php
                    $COMM2a = "Directive";
                    $COMM2b = "Collaborative";
                    @endphp
                    @else
                    @php
                    $COMM2a = "Directive";
                    ...
                    <p>{{$primary_decision->primary_decision}}</p>
                    <p>{{$secondary_decision->secondary_decision}}</p>

                    <h3>Decision Making Shadow</h3>
                    <p>{{$primary_decision_shadow->primary_decision_shadow}}</p>
                    <p>{{$secondary_decision_shadow->secondary_decision_shadow}}</p>

                    <h3>Decision Making Remedy</h3>
                    <p>{{$primary_decision_remedy->primary_decision_remedy}}
                    <p>{{$secondary_decision_remedy->secondary_decision_remedy}}
                    ...
                    $check_page = DB::table('progress_steps')->where('formid',$datasession['formid'])->where('page13','!=',NULL)->first();
                    @endphp
                    @if($check_page)
                    ...
                    <form method="post" action="/markcomplete">
                        @csrf
                        <input type="hidden" name="page" value="page13">
